...
class Dataset(data.Dataset):
    ...
    def __init__(self, file_name, logger, opt, training=True, pn_mode=None):
        ...
        self.processed_data_name = opt['dataset'] + '-' + opt['MGSA_encoder'] + '-' + self.file_name.split(".")[0] + '.pkl'
    ...
    def bert_processor(self, bag, ins):
        ...
        # mask
        _mask = np.zeros(self.max_length, dtype=np.int64)  
        _mask[:length] = 1
    ...
    def vanilla_processor(self, bag, ins):
        ...
        # mask
        p1, p2 = sorted((p1, p2))
        _mask = np.zeros(self.max_length, dtype=np.int64)  # 修改为 np.int64
