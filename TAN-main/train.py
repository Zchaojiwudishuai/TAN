...
import sys
import os
from TAM import compute_embedding_difference
...
def train(train_loader, test_loader, logger, opt):
    ...
    model = TAN(word_vec_dir, edge, constraint, type_num, rel_num, opt)
    if torch.cuda.is_available():
        model = model.cuda()
    criterion = nn.CrossEntropyLoss(weight=train_loader.dataset.rel_weight())
    if opt['MGSA_encoder'].lower() == 'bert':
    ...
    for epoch in range(opt['epoch']):
        ...
        for i, data in enumerate(train_loader):
            ...
            sys.stdout.write(
                '\rstep: %d | loss: %f, acc: %f, pos_acc: %f' % (i + 1, avg_loss.avg, avg_acc.avg, avg_pos_acc.avg))
            sys.stdout.flush()
            # Optimize
            loss.backward()
            optimizer.step()
            ...
            embeddings.update(model.collect_embeddings(data))

        logger.info('\n[Train] loss: %f, acc: %f, pos_acc: %f' % (avg_loss.avg, avg_acc.avg, avg_pos_acc.avg))

        if epoch % opt['affinity_update_interval'] == 0:
            model.compute_affinity(embeddings)
            embedding_diff = compute_embedding_difference(embeddings)
            model.update_type_constraints(embedding_diff)
